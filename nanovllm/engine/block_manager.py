...
class BlockManager:
    ...
    def append_n_slots(self, seq: Sequence, n: int, start_pos: int | None = None) -> list[int]:
        """预分配 n 个 future token 的 blocks 并返回 slot 列表（不修改 seq.num_tokens）"""
        slots = []
        temp_len = len(seq) if start_pos is None else start_pos
        for _ in range(n):
            block_idx = temp_len // self.block_size
            while block_idx >= len(seq.block_table):
                block_id = self.free_block_ids[0]
                self._allocate_block(block_id)
                seq.block_table.append(block_id)
