...
class ModelRunner:
    ...
    def _run_spec_proposal_batch(self, proposal_infos: list[SpecProposalInfo], sync_logits: torch.Tensor,
        ...
        # sync 最后一行的 logits 给出第一个 draft token；hidden 作为后续 serial draft loop 的初始 state。
        ...
        draft_slots = None
        if K > 1:
            # proposal loop 会 forward K-1 次：d0->d1, ..., d_{K-2}->d_{K-1}。
            # 这里一次性按每个 seq 的 start_position 预留 K-1 个 slots。
            draft_slots = [
    ...
    def _build_spec_target_batch(self, prefill_seqs: list[Sequence], decode_seqs: list[Sequence]):
        ...
        for prefill_index, seq in enumerate(prefill_seqs):
            ...
            # start/end 是 seq 内部的 token 位置， 主要就是给 draft sync 和 seq 逻辑位置相关的操作
            # start/end：给 seq 逻辑位置、draft sync、position、slot mapping 用
            rows.append({
                "kind": "prefill",
                "seq": seq,
    ...
    def _build_spec_draft_sync_batch(self, rows: list[dict], input_ids: torch.Tensor, captured: dict[int, torch.Tensor]):
        ...
        for row in rows:
            ...
            if row["kind"] == "prefill":
                ...
                if will_finish:
                    # prev_draft_tokens 是给下一轮 spec verify 用的草稿 token。  
                    # 如果这个 seq 已经结束：没有下一轮 verify 所以要清空：

                    seq.prev_draft_tokens = []
                    continue

                row_sync_start = len(sync_input_ids)
                # accepted 保存的是 target 确认后的“下一位 token”： 已经shifted
    ...
    def _run_spec_draft_sync_and_propose(self, sync_input_ids: list[int], sync_positions: list[int],
        ...
        if sync_input_ids:
            # sync_* 是 draft model 的一次 varlen prefill：用 target captured hidden + shifted token ids
            # 把 draft KV 同步到 target 已确认的位置，再从最后一个 sync row 继续 proposal。
            ...
            # cu_k > cu_q 表示 attention 需要看历史 KV cache，所以需要 block_tables 找历史 blocks。
            sync_block_tables = self.prepare_block_tables(sync_seqs) if sync_cu_k[-1] > sync_cu_q[-1] else None

            # 跑 draft sync：写 draft KV cache，并返回每个 sync row 的 logits / hidden。
            ...
            # sync 是一个 batch；proposal 也整理成一个 batch，但是partial prefill 不参与了
            # 只是 prefill/decode 的 metadata 来源不同。
            proposal_infos = []
            for sync_row in sync_rows:
                source = sync_row["source_row"] # 对应 target 的 row
    ...
    def capture_spec_decode_cudagraph(self):
        ...
        for bs in reversed(self.spec_graph_bs):
            ...
            set_context(True, cu_q[:bs + 1], cu_k[:bs + 1], q_len, self.spec_graph_max_seqlen_k,
                        ...
                        finishing_prefill_indices=[])
            forward_graph() # warmup / dry run
            graph = torch.cuda.CUDAGraph()
            with torch.cuda.graph(graph, self.spec_graph_pool):
                forward_graph()  # capture
            if self.spec_graph_pool is None:
                self.spec_graph_pool = graph.pool()
            self.spec_graphs[bs] = graph
            #  是等 GPU 把当前 graph capture / warmup 相关工作全部做完
            # 确保当前 bs 的 graph capture 完整结束，再开始下一张 bs graph 的 capture
            torch.cuda.synchronize()
            reset_context()
